...
sUserName = oConfig.SelectUser("ebs", "test2")
sPassword = oConfig.SelectPassword("ebs", "test2")
...
objEmail.From = "dl.GTS.vb.test.dev.Support@imcnam.ssmb.com"
objEmail.To = "dl.GTS.vb.test.test2@imcnam.ssmb.com"
...
'Function TRXMZDSG()
	...
	' If Not IsEmpty(controlrs14) And Not IsNull(controlrs14.fields("SUPER_COUNT_Japan")) And controlrs14.fields("SUPER_COUNT_Japan") <>0 Then
		...
		' 'fun_LOGGER("Insert into vbCOM.DAILY_LOAD_EVENT (SUBID,PROC_CCID,FILE_NAME,FILE_DATE,TXNS_POST_DATE,START_DATE,END_DATE,EVENT_TRIGGER_DATE,EVENT_TRIGGER_FLAG) select SUBID,PROC_CCID,FILE_NAME,FILE_DATE,TXNS_POST_DATE,START_DATE, END_DATE,sysdate as EVENT_TRIGGER_DATE,'Y' AS EVENT_TRIGGER_FLAG From CONTROL.PROCESSOR_FILE where file_name like '%TRXMZDSG%'and date(TXNS_POST_DATE)=(current date -1 DAYS) and End_DATE is not NULL ;")
	' Else
		' Loadcomplete = 0
